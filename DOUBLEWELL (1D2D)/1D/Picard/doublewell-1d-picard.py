"""Solve the 1D double-well MFG with the Picard method."""
...
# --- Saved File Names ---
history_filename = "doublewell-1d-picard-history.txt"
plot_snapshot_filename = "doublewell-1d-picard-plot.png"
...
def solve_fp_forward(U_flow):
    ...
    for n in range(Nt):
        ...
        dp, dm = D_plus @ u_curr, D_minus @ u_curr
        J_H = (sp.diags(dH_dp(np.minimum(dp, 0))) @ D_plus +
               sp.diags(dH_dp(np.maximum(dm, 0))) @ D_minus).tocsr()
        # The FP transport is the exact transpose of the discrete HJB
        # Hamiltonian Jacobian.  Together with the periodic Laplacian, this
        # gives a conservative density update.
...
with open(history_filename, "w", buffering=1) as f_log:
    header = (
        f"{'='*118}\n"
        f"   DOUBLE WELL MFG EXAMPLE (Picard)\n"
        f"{'='*118}\n"
        f"Parameters:\n"
