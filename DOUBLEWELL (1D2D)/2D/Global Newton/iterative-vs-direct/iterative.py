...
Z_s = sp.csc_matrix((num_s, num_s)) # <-- Added the missing zero matrix block
...
print(f"--- GLOBAL NEWTON ITERATIVE STUDY (Nx={Nx}, Nt={Nt}) ---")
t_build = time.time()
...
t_ilu = time.time()
try:
    print("Computing ILU Preconditioner (This may take a minute)...")
    # Diagnostic for Incomplete LU
    ilu = spla.spilu(J, drop_tol=1e-5, fill_factor=10)
    ilu_time = time.time() - t_ilu
    
    print("Starting BiCGSTAB Solve (This will take a while)...")
    M_precond = spla.LinearOperator(J.shape, ilu.solve)
    t_bicg = time.time()
    iteration_count = 0
    def count_iteration(_):
        global iteration_count
        iteration_count += 1
    step, info = spla.bicgstab(
        J, -F, M=M_precond, tol=1e-6,
        maxiter=300, callback=count_iteration
    )
    solve_time = time.time() - t_bicg

    print(f"\nTotal DOFs:      {J.shape[0]}")
    print(f"Jacobian NNZ:    {J.nnz}")
    print(f"ILU Factor NNZ:  {ilu.nnz}")
    print(f"ILU Build Time:  {ilu_time:.4f}s")
    print(f"Solve Time (s):  {solve_time:.6f}")
    print(f"Iterations:      {iteration_count}")
    print(f"Relative Residual: {np.linalg.norm(J @ step + F) / np.linalg.norm(F):.6e}")
    print(f"Iterative Status: {'Success' if info == 0 else f'Failed (info={info})'}")
except Exception as e:
    print(f"\n>>> ITERATIVE STUDY FAILED: {e}")
